...
public class Dispatcher extends Controller{
	...
	/*	if(curr_f == 1)
						...
						for (Hallway h : Hallway.replicationValues) {
							...
							if(mCarCall[index].getValue() || mHallCall[hallIndex].getValue()){
								...
								if(!targetFound && commitPoint(f, current_d, car_position, speed)){
									target = f;
									targetFound = true;
									//System.out.printf("target set to:%d\n", target);
									if (f == 1) nextHallCall = Direction.UP;
									else if (f == Elevator.numFloors) nextHallCall = Direction.DOWN;
		...
		//System.out.printf("curr_f:%d\ttargetFound:%b\ttarget:%d\tnextTargetFound:%b\tnextTarget:%d\n",
			//	curr_f, targetFound, target, nextTargetFound, nextTarget);
